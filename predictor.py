...
from ExponentialSmooth import es_predict
from tool_lib import Tool

from predict_magic import predict_magic
...
def data_process(ecs_lines, input_lines):
    ...
    if input_contents[virtual_num + 4].split('\r')[0] == 'CPU':
        cpu_if = 1
    else:
        cpu_if = 0

    start_date_temp = input_contents[virtual_num + 6].split('\r')[0][:10]
    # start_date = [int(start_date_temp[:4]), int(start_date_temp[5:7]), int(start_date_temp[8:])]
    end_date_temp = input_contents[virtual_num + 7].split('\r')[0][:10]
    # end_date = [int(end_date_temp[:4]), int(end_date_temp[5:7]), int(end_date_temp[8:])]

    start_date = datetime.datetime.strptime(start_date_temp, '%Y-%m-%d')
    end_date = datetime.datetime.strptime(end_date_temp, '%Y-%m-%d')
    predict_days_num = (end_date - start_date).days

    ecs_lines = ecs_lines[::-1]

    ecs_data_lr = {}

    for line in ecs_lines:
        if len(ecs_data_lr) <= predict_days_num * 20:
            ecs_contents = line.split('\t')
            flavor = int(ecs_contents[1][6:])
            if flavor <= 15:
                # date = time.mktime(time.strptime(ecs_contents[2].split('\r')[0],"%Y-%m-%d %H:%M:%S"))
                date = ecs_contents[2].split('\r')[0][:10].replace('-', '')
                # date = ecs_contents[2].split('\r')[0][:13].replace(' ', '-')
...
def predict(lg_count_list, lr_count_list, rf_count_list, es_count_list, virtual_info, lg_round, mix_rf, mix_lr, mix_es,
    ...
    for flavor, info in virtual_info.items():
        ...
        for i in range(lg_round):
            lg_window.append(
                ...
                Tool.mid([lg_count_list[j][flavor - 1] for j in range(i - 3, i + 3) if 0 <= j < lg_round]))
        ...
        for i in range(20 - lr_day_gap + 1):
            for j in range(1, lr_day_gap):
                ...
                lr_window.append(Tool.mid(
                    [lr_count_list[i + jj][int(flavor) - 1] for jj in range(j - 3, j + 3) if 0 <= jj < lr_day_gap]))
    ...
    for key, value in predict_result.items():
        ...
        predict_result[key] = int(
            ...
            (1 - mix_lr - mix_rf - mix_es) * lg_predict_result[key] + floor)  # 四舍五入
        # predict_result[key] = int(
        #     mix_lr * lr_predict_result[key] + mix_rf * rf_predict_result[key] + mix_es * es_predict_result[key] +
...
def predict_vm(ecs_lines, input_lines):
    ...
    # rf_count_list = magic(ecs_data_lr, start_date, early_round=sum_of_days, interval=8)  # true 8
    es_count_list = magic(ecs_data_lr, start_date, early_round=20, interval=predict_days_num)
    # es_count_list = no_slide_magic(ecs_data_lr, start_date, early_round=15, interval=predict_days_num)

    predict_result = predict_magic(lg_count_list, es_count_list, virtual_info, 20,
                                   mix_rf=1, mix_lr=0, mix_es=0,
